...
module.exports = function(mp, config, init, pageObject) {
    /**
     * 处理一些特殊的页面
     */
    ...
    const pageOptions = {
        ...
        onShow() {
			...
			if(pageObject && pageObject.onShow){
				pageObject.onShow();
			}
        },
        onReady() {
			...
			if(pageObject && pageObject.onReady){
				pageObject.onReady();
			}
        },
        onHide() {
        ...
        onUnload() {
            ...
            this.app = null;
            this.query = null;
			if(pageObject && pageObject.onUnload){
				pageObject.onUnload();
			}
        },
        onShareAppMessage(data) {
			if(pageObject && pageObject.onShareAppMessage){
				return pageObject.onShareAppMessage(data);
			}
        ...
        onAddToFavorites(data) {
			if(pageObject && pageObject.onAddToFavorites){
				return pageObject.onAddToFavorites(data);
			}
        ...
        onResize() {
			...
			if(pageObject && pageObject.onResize){
				pageObject.onResize();
			}
        },
        onTabItemTap(data) {
			...
			if(pageObject && pageObject.onTabItemTap){
				pageObject.onTabItemTap(data);
			}
        },
    }
    ...
    if (compareVersion(version, '2.11.3') >= 0) {
        pageOptions.onShareTimeline = function() {
			if(pageObject && pageObject.onShareTimeline){
				return pageObject.onShareTimeline();
			}
